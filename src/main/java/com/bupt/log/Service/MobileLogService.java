...
public class MobileLogService {
    ...
    //查询所有日志
    public Page<MobileLog> queryAll(){
        Query query=new Query();
        Pageable pageable=new PageRequest(0,10);
        query.with(pageable);
        List<MobileLog> target=mongoTemplate.find(query,MobileLog.class);
        return (PageImpl<MobileLog>) PageableExecutionUtils.getPage(target, pageable, () -> 0);
    }
    //根据uuid查找日志
    public Page<MobileLog> queryByUuid(String uuid){
    ...
    //根据日期范围查找日志
    public Page<MobileLog> queryByDate(String start,String end) throws Exception{
    ...
    //根据ip地址查找日志
    public Page<MobileLog> queryByIp(String ip){
